...
class QTable(gridlib.Grid):
    def __init__(self, parent):
        ...
        self.oddRow.SetBackgroundColour(wx.Colour(245, 245, 245))
    ...
    def Create(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.WANTS_CHARS,
               name=gridlib.GridNameStr):
        super().Create(parent, id, pos, size, style, name)
    ...
    def create(self):
        ...
        if self._initialRows > 0:
            self.height_row = int(self.GetRowSize(1) * 1.25)  # 25% more on row heights

        self.SetRowLabelSize(40)  # set row labels size

        self.SetSelectionMode(gridlib.Grid.SelectCells)  # select the whole row
        ...
        gridWin = self.GetGridWindow()
        gridWin.Bind(wx.EVT_MOTION, self.OnGridMotion)
        self.GetParent().Bind(wx.EVT_MOTION, self.OutGridMotion)
        if hasattr(self, '_validator_fn'):
    ...
    def OnGridMotion(self, evt):
        ...
        self.SelectRow(row)
        self.SetSelectionBackground(wx.Colour(229, 238, 204))

    def OutGridMotion(self, evt):
        self.SelectRow(-1)
